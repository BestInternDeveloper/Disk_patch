...
{
    public LineChartDemo2(String s, ArrayList<Integer> list31,
                                    ArrayList<Integer> list41,
                                    ArrayList<Integer> list3,
                                    ArrayList<Integer> list4,int TracksNum)
    {
        super(s);
        CategoryDataset categorydataset = createDataset(list31,list41,list3,list4,TracksNum);
        JFreeChart jfreechart = createChart(categorydataset);
        ChartPanel chartpanel = new ChartPanel(jfreechart);
        chartpanel.setPreferredSize(new Dimension(800, 600));
        setContentPane(chartpanel);
    }
    private static CategoryDataset createDataset(ArrayList<Integer> list31,
                                                    ArrayList<Integer> list41,
                                                    ArrayList<Integer> list3,
                                                    ArrayList<Integer> list4,int TracksNum)
    ...
    {
        String s1 = "SCAN--Left";
        String s2 = "CSCAN--Left";
        String s3 = "SCAN--Right";
        String s4 = "CSCAN--Right";
        //s3-s10   (type3-type10)
        ArrayList<String> Types = new ArrayList<String>();
        for(int i = 0;i<TracksNum;i++){
            Types.add("Time "+i);
            System.out.println(Types.get(i));   
            System.out.println(list31.get(i));
        }  
        DefaultCategoryDataset defaultcategorydataset = new DefaultCategoryDataset();
        for(int i=0;i<TracksNum;i++){
            defaultcategorydataset.addValue(list31.get(i), s1, Types.get(i));
        }
        for(int i=0;i<TracksNum;i++){
            System.out.println(list41.get(i));
            defaultcategorydataset.addValue(list41.get(i), s2, Types.get(i));
        }
        for(int i=0;i<TracksNum;i++){
